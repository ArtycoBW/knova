...
export const Radar = ({ className }: { className?: string }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative z-40 h-[1px] w-full bg-linear-to-r from-transparent via-emerald-500 to-transparent" />
      </div>
      {circles.map((_, idx) => (
        <Circle
          style={{
...
export const IconContainer = ({
  ...
  return (
    ...
    >
      <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-gray-200 bg-gray-100 shadow-inner dark:border-slate-700 dark:bg-slate-900">
        {icon || (
          <svg
      ...
      <div className="hidden rounded-md px-2 py-1 md:block">
        <div className="text-center text-xs font-bold text-gray-500 dark:text-slate-400">
          {text || "Feature"}
        </div>
      </div>
    </motion.div>
