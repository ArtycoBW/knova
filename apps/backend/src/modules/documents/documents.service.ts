...
export class DocumentsService {
  ...
  async upload(
    ...
    if (kind === "document") {
      await this.docQueue.add("process", { documentId: document.id }, { attempts: 3, backoff: { type: "exponential", delay: 5000 } });
    } else {
      await this.audioQueue.add("process", { documentId: document.id }, { attempts: 3, backoff: { type: "exponential", delay: 5000 } });
    }
